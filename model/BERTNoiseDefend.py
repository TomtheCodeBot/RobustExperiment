...
from transformers.modeling_outputs import BaseModelOutputWithPastAndCrossAttentions,BaseModelOutputWithPoolingAndCrossAttentions,SequenceClassifierOutput,MultipleChoiceModelOutput
import math
import os
import warnings
...
class BertLayer(nn.Module):
    def __init__(self, config,defense = False,def_position=False,defense_cls='identical',noise_sigma=1e-3):
        super().__init__()
...
class BertEncoder(nn.Module):
    def __init__(self, config,def_position=None,defense_cls='identical',noise_sigma=1e-3,defense=False):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([BertLayer(config,def_position,defense_cls,noise_sigma,defense) for _ in range(config.num_hidden_layers)])
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.encoder = BertEncoder(config,def_position=None,defense_cls='identical',noise_sigma=1e-3,defense=False)
        self.defense = False
        self.def_position = None
    ...
    def forward(
        ...
        if self.defense == True and self.def_position == "input_noise":
            embedding_output = self.defense_token(embedding_output)
