...
class RobustNaiveBayesMultiClassifierPercentage(BaseEstimator):
    ...
    def predict_proba(self, X):
        ...
        result = []
        for i in range(self.num_classes):
            result.append(np.expand_dims(np.exp(np.log(
        ...
        result[0] = np.nan_to_num(result[0], nan=min_value)
        result[0][result[0] == 0] = min_value
        exp_sum = np.copy(result[0])
        for i in range(1, self.num_classes):
            result[i] = np.nan_to_num(result[i], nan=min_value)
            result[i][result[i] == 0] = min_value
            exp_sum += result[i]

        exp_sum = np.squeeze(exp_sum)
