...
def deviation_from_number(tensor, number):
    deviation = torch.abs(tensor - number)
...
if __name__ == "__main__":
    ...
    train_data,train_label,_,_=train_test_split(test_data["text"],test_data["label"],train_size=1000)
    train_data = list(train_data)
    for i in tqdm(range(len(train_data))):
        model.change_defense(defense=False)
        original_result = torch.nn.functional.softmax(BERT([train_data[i]]))
        index = torch.argmax(original_result[0])
        original_result = original_result[0][index]
        model.change_defense(defense_cls="random_noise",def_position="post_att_cls",noise_sigma=0.8,defense=True)
        res = BERT([train_data[i]]*32)
        res = torch.nn.functional.softmax(res, dim=1)
        std.append(deviation_from_number(res[:,index],original_result).item())
