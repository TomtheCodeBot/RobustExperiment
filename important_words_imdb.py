...
if __name__ == "__main__":
    ...
    important_ind2 = []
    model.change_defense(defense_cls="random_noise",def_position="post_att_cls",noise_sigma=0.55,defense=True)
    BERT = HuggingFaceModelWrapper(model, tokenizer_tmd)
    attack  =  build_attacker_from_textdefender(BERT,args)
    for i in tqdm(range(len(test_data["text"][:1000]))):
        example = AttackedText(test_data["text"][i])
